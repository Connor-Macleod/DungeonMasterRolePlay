...
local function splitAndSendChat(message, channel, language, ...)
    local messageLength = #message
    local messages = {}
    if messageLength >= 255 then
        local words = DMRP.Dice.spreadSlashArgs(message);
        local i = 1;
        for j, msg in ipairs(words) do
            if not messages[i] then messages[i] = '' end
            if #(messages[i]..msg) > 200 then
                i = i+1
                end
        end
    else
